...
function processCsv(csv){
        ...
        for(const difficulty of difficulties){
            if (song[difficulty + "_score"] > 0){
                ...
                if(master_data_song[key]){
                    temp["rate"] = (song[difficulty + "_score"] / master_data_song[key]["notes"] / 2);
                    temp["notes"] = master_data_song[key]["notes"];
                    temp["max-"] = master_data_song[key]["notes"]*2 - song[difficulty + "_score"];
                } else {
                    temp["rate"] = 0.0;
                    temp["notes"] = 1;
                    temp["max-"] = 9999;
    ...
    for (let i = 12; i >= 1; i--) {
        ...
        stastics.push("<td>" + stats[i]["played"] + "/" + stats[i]["total"] + "</td>");
        stastics.push("<td>" + (stats[i]["average_rate"]*100).toFixed(3) + "%</td>");
    ...
    for (let i = 12; i >= 1; i--) {
        let temp = "";
        if(i === 12){
            temp += "☆12 avg: " + (stats[i]["average_rate"]*100).toFixed(2) + "% (" + stats[i]["played"] + "/" + stats[i]["total"] +") | ";
            ...
            temp += "☆11 avg: " + (stats[i]["average_rate"]*100).toFixed(2) + "% (" + stats[i]["played"] + "/" + stats[i]["total"] +") | ";
            temp += "max-**: " + stats[i]["2keta"] + " / ";
            temp += "99%: " + stats[i]["99%"] + " / ";
            temp += "98%: " + stats[i]["98%"] + " / ";
            temp += "97%: " + stats[i]["97%"] + " / ";
